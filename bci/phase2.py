...
class SubjectAdversarialHead(nn.Module):
    ...
    A single hidden layer is enough for the small BCI-IV 2a subject count. A
    deeper discriminator can over-optimize subject removal and damage class
    separability.
    """
...
class ContrastiveProjectionHead(nn.Module):
    ...
    P4-FIX: Widened projection from 128Ã¢â€ â€™256 for better class separability
    in 4-class BCI classification.
    """
...
class EEGNet(nn.Module):
    def __init__(
        ...
        _D = 2  # P95v3: D=2 Ã¢â‚¬â€ D=3 caused subject memorization with 6192 samples
        ...
        self.head = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(min(self.dropout * 1.0, 0.50)),
        )
        self.pool = nn.AdaptiveAvgPool1d(self.pool_out)
        self.pre_classifier = nn.Sequential(
            nn.Flatten(),
            nn.Linear(self.fusion_channels * self.pool_out, self.fusion_channels),
            nn.LayerNorm(self.fusion_channels),  # CROSS-SUBJECT: stabilize feature magnitudes
            nn.GELU(),
            nn.Dropout(min(self.dropout * 1.0, 0.50)),
        )
        self.classifier_dropout = nn.Dropout(min(self.dropout * 1.0, 0.50))
    ...
    def set_dropout_scale(self, scale: float) -> None:
        ...
        for module, base_p in self._dropout_modules:
            module.p = min(max(base_p * scale, 0.0), 0.75)  # v24: cap at 0.75 for hard-subject adaptive dropout

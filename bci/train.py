...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--epochs", type=int, default=300, help="v18: 300 â€” slower lr needs more epochs.")
    parser.add_argument("--batch-size", type=int, default=32)
    parser.add_argument("--lr", type=float, default=5e-4, help="v18: 5e-4 â€” 1e-3 caused overfitting by epoch 60.")
    parser.add_argument("--weight-decay", type=float, default=0.05, help="v18: 0.05 â€” stronger L2 to prevent memorization.")
    parser.add_argument("--hidden-channels", type=int, default=32, help="v18: 32 restored â€” 48 gave too much capacity, train hit 98%.")
    parser.add_argument("--dropout", type=float, default=0.50, help="v20: 0.50 restored â€” v19 proved 0.40 caused worse overfitting (train-val gap 41% vs 34%). Stronger dropout needed with cosine_restarts.")
    parser.add_argument("--scheduler", choices=("onecycle", "cosine", "cosine_restarts", "cosine_single_restart"), default="cosine_restarts", help="v19: cosine_restarts â€” periodic LR resets escape val loss plateaus that killed OneCycle.")
    parser.add_argument("--loss", choices=("weighted_ce", "focal"), default="focal")
    parser.add_argument("--focal-gamma", type=float, default=0.0, help="v19: 0.0 = standard CE. gamma=0.5 caused AUC-accuracy gap by softening gradients on classifiable samples.")
    ...
    parser.add_argument("--label-smoothing", type=float, default=0.10, help="v18: 0.10 â€” prevents overconfident predictions that cause val loss divergence.")
    parser.add_argument("--cutmix-alpha", type=float, default=0.0, help="v16: disabled â€” CutMix corrupts temporal EEG phase structure.")
    parser.add_argument("--cutmix-prob", type=float, default=0.0, help="v16: disabled.")
    parser.add_argument("--rdrop-alpha", type=float, default=0.03, help="v19: 0.03 â€” 0.05 fought cosine restart re-exploration. 0.03 still fixes calibration without resisting LR resets.")
    parser.add_argument("--time-mask-prob", type=float, default=0.3, help="v18: restored to v16 level.")
    parser.add_argument("--time-mask-ratio", type=float, default=0.1)
    parser.add_argument("--time-masks-per-sample", type=int, default=1)
    parser.add_argument("--noise-prob", type=float, default=0.40, help="v18: restored â€” need augmentation to prevent memorization.")
    parser.add_argument("--noise-std", type=float, default=0.03, help="v18: restored to v16 level.")
    parser.add_argument("--channel-dropout-prob", type=float, default=0.15, help="v18: restored to v16 level.")
    parser.add_argument("--channel-dropout-ratio", type=float, default=0.15)
    parser.add_argument("--temporal-shift-prob", type=float, default=0.3, help="v18: restored to v16 level.")
    parser.add_argument("--temporal-shift-ratio", type=float, default=0.05)
    parser.add_argument("--scaling-prob", type=float, default=0.3, help="v18: restored to v16 level.")
    ...
    parser.add_argument("--patience", type=int, default=80, help="v18: 80 â€” let OneCycle complete its full cycle.")
    parser.add_argument("--min-delta", type=float, default=1e-4)
    parser.add_argument("--ema-decay", type=float, default=0.9995, help="v16: slower EMA for stability.")
    ...
    parser.add_argument("--tta-passes", type=int, default=15)
    ...
    parser.add_argument("--num-folds", type=int, default=9)
    ...
    parser.add_argument("--aug-warmup-epochs", type=int, default=0)  # P95v3: no warmup Ã¢â‚¬â€ full aug from epoch 1
    ...
    parser.add_argument("--dann-lambda", type=float, default=0.0, help="v19: DISABLED. 0.03 in last run helped Folds 1-3 marginally but not hard subjects.")
    parser.add_argument("--dann-warmup-epochs", type=int, default=15, help="v16: earlier gentle activation.")
    # NOVEL: Supervised contrastive loss for class separability
    parser.add_argument("--supcon-weight", type=float, default=0.01, help="v19: 0.01 â€” 0.05 in last run was too noisy with batch=32 (only ~8 same-class pairs).")
    # P95v3: SAM disabled by default
    parser.add_argument("--use-sam", type=str, default="false", help="P95v3: disabled Ã¢â‚¬â€ no benefit at this model size.")
    ...
    parser.add_argument("--head-lr-mult", type=float, default=1.0, help="v19: 1.0 â€” with cosine_restarts, head was getting 0.25x of already-tiny LR. Same rate as backbone now.")
    # P95: MC-Dropout inference
    parser.add_argument("--mc-dropout-passes", type=int, default=10, help="P15: more MC-Dropout for 4-class.")
    # P95: Frequency augmentations
    parser.add_argument("--freq-aug-prob", type=float, default=0.0, help="v16: disabled â€” corrupts spectral view consistency.")
    return parser
...
class TrainConfig:
    ...
    loss: str = "focal"
    focal_gamma: float = 0.0              # v17: standard CE
    mixup_alpha: float = 0.0
    label_smoothing: float = 0.10
    cutmix_alpha: float = 0.0             # v16: disabled â€” corrupts temporal EEG
    cutmix_prob: float = 0.0
    rdrop_alpha: float = 0.03
    # Augmentation
    ...
    channel_dropout_ratio: float = 0.15
    ...
    aug_warmup_epochs: int = 0           # P95v3: NO warmup Ã¢â‚¬â€ full augmentation from epoch 1
    aug_rampup_epochs: int = 10          # P95v6: slightly longer rampup
    balanced_sampling: bool = False
    # NOVEL: Domain Adversarial Training (DANN)
    dann_lambda: float = 0.0        # v17: disabled
    dann_warmup_epochs: int = 15     # v16: earlier activation
    # NOVEL: Supervised contrastive loss
    supcon_weight: float = 0.01
    # P95v3: SAM disabled Ã¢â‚¬â€ caused instability, no benefit at this model size
    use_sam: bool = False
    sam_rho: float = 0.02
...
def _unique_subject_set(subjects: np.ndarray) -> set[int]:
    return {int(value) for value in np.unique(np.asarray(subjects).reshape(-1)).tolist()}
...
def _validate_subject_splits(
    *,
    ...
    train_set = _unique_subject_set(train_subjects)
    val_set = _unique_subject_set(val_subjects)
    calib_set = _unique_subject_set(calib_subjects)
    test_set = _unique_subject_set(test_subjects)

    invalid_overlaps = {
        "train_val": train_set & val_set,
        "train_calib": train_set & calib_set,
        "train_test": train_set & test_set,
        "val_test": val_set & test_set,
        "calib_test": calib_set & test_set,
    }
    invalid_overlaps = {key: sorted(values) for key, values in invalid_overlaps.items() if values}
    if invalid_overlaps:
        raise ValueError(f"Subject leakage detected in fold split: {invalid_overlaps}")
...
def plot_confusion_matrix(cm: np.ndarray, class_names: list[str], save_path: Path) -> None:
    if plt is None:
        LOGGER.warning("matplotlib is unavailable; skipping confusion matrix plot: %s", save_path)
        return
...
class SAM(torch.optim.Optimizer):
    ...
    def step(self, closure=None):
        # For compatibility Ã¢â‚¬â€ SAM uses first_step/second_step explicitly
...
def apply_bandstop_noise(
    ...
    """P95: Band-stop noise Ã¢â‚¬â€ randomly attenuate a narrow frequency band.
    ...
    # FFT Ã¢â€ â€™ attenuate random band Ã¢â€ â€™ iFFT
...
def apply_phase_perturbation(
    batch_x: torch.Tensor | dict[str, torch.Tensor],
    prob: float = 0.3,
    max_phase_shift: float = 0.4,  # ~Ã‚Â±Ãâ‚¬/8 radians
) -> torch.Tensor | dict[str, torch.Tensor]:
    """P95: Phase perturbation Ã¢â‚¬â€ add small random phase shifts to FFT coefficients.
...
def train_one_epoch(
    ...
    if use_dann:
        # P15v2 FIX: GRL is a pure sign-flipper (lambda=1.0).
        # The dann_lambda_val sigmoid ramp is applied when ADDING dann_loss to
        # the total loss (see below), keeping the loss scale stable.
        # Previous design: set_lambda(dann_lambda_val) caused the loss to be
        # added unscaled (~1.94 for 7 subjects), tripling the total loss at warmup.
        model.subject_adversarial.set_lambda(1.0)
    ...
    for batch_x, batch_y, batch_subj in loader:
        ...
        with autocast_context(device, enabled=cfg.use_amp):
            ...
            if use_dann and features is not None and batch_subj is not None:
                ...
                if not (torch.isnan(dann_loss) or torch.isinf(dann_loss)):
                    # P15v2 FIX: GRL=1.0 (pure sign flip). dann_lambda_val (sigmoid-ramped)
                    # weights the loss contribution. At warmup epoch, dann_lambda_valâ‰ˆ0.09
                    # so contribution = 0.09Ã—1.94=0.17 (vs old design: raw 1.94 added).
                    # This prevents the epoch-36 train_loss spike from 1.0â†’2.9.
                    loss = loss + dann_lambda_val * dann_loss
        ...
        if sam_active and cfg.use_amp:
            ...
            # Step 1: compute gradient at current weights Ã¢â€ â€™ perturb to worst-case
            ...
            # Step 2: compute gradient at perturbed weights Ã¢â€ â€™ apply actual update
            with autocast_context(device, enabled=cfg.use_amp):
                if use_cutmix:
            ...
            scaler.unscale_(optimizer.base_optimizer)  # official unscale Ã¢â‚¬â€ only call between update()s
            clip_grad_norm_(model.parameters(), cfg.max_grad_norm)
...
def predict_logits_with_tta(
    ...
    *,
    ...
    # Standard TTA passes (eval mode Ã¢â‚¬â€ deterministic dropout)
    model.eval()
    ...
    # P95: MC-Dropout passes (train mode Ã¢â‚¬â€ stochastic dropout masks)
...
def _build_scheduler(
    ...
    if scheduler_name == "cosine_single_restart":
        # P95: Single restart cosine Ã¢â‚¬â€ two-phase training with one strategic restart
        # Phase 1: warmup(10ep) Ã¢â€ â€™ cosine decay to eta_min over first half
        # Phase 2: jump back to peak LR Ã¢â€ â€™ cosine decay to eta_min over second half
...
def save_checkpoint(
    ...
    # Also remove SWA internal keys like 'n_averaged' that don't belong in EEGNet
    state_dict = model.state_dict()
    swa_internal_keys = {"n_averaged"}
    sanitized = {
        k.replace("module.", "", 1) if k.startswith("module.") else k: v
        for k, v in state_dict.items()
        if k not in swa_internal_keys and not k.startswith("module.n_averaged")
    }
    torch.save(
        {
...
def _run_training_session(
    *,
    ...
    preprocessing = metadata.get("preprocessing", {}) if isinstance(metadata.get("preprocessing"), dict) else {}
    if float(preprocessing.get("high", 0.0) or 0.0) > 30.0 and not bool(preprocessing.get("include_gamma", False)):
        LOGGER.warning(
            "[%s] Prepared data uses high cutoff %.1f Hz without gamma bands. "
            "For BCI motor imagery, rerun preprocessing with --high 30.0.",
            fold_name,
            float(preprocessing.get("high")),
        )
    ...
    # P90: Differential weight decay Ã¢â‚¬â€ backbone gets cfg.weight_decay (2e-3),
    ...
    if use_sam:
        # P95: SAM wraps AdamW for flatter minima Ã¢â€ â€™ better cross-subject generalization
        optimizer = SAM(
            [
        ...
        # Head LR multiplier Ã¢â‚¬â€ controllable from CLI via --head-lr-mult
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        # === P15v2: Hard-Subject Adaptive LR ===
        # Detect folds where cross-subject generalization is fundamentally poor
        # (e.g. BCIC-IV-2a Subject 3 has near-zero motor imagery SNR).
        # At epoch 25: if val_auc < 0.58 â†’ disable DANN (it hurts bad subjects)
        #              and halve LR to reduce overfitting speed.
        # At epoch 50: if val_auc < 0.60 â†’ tighten patience to 20 so we don't
        #              waste 100+ epochs when the fold is clearly not converging.
        # v19: Moved from ep25â†’ep50. With cosine_restarts (T_0=25), the first
        # cycle ends at ep30 â€” at ep25 the LR is near eta_min and AUC is always
        # <0.58 for EVERY fold. This was halving LR on ALL 9 folds, crippling training.
        if epoch == 40 and best_auc < 0.58:
            print(
                f"[{fold_name}] Hard-subject fold detected (best_val_auc={best_auc:.4f} < 0.58 at ep 40). "
                f"Disabling DANN and boosting dropout for this fold."
            )
            # Boost dropout for hard subjects to slow memorization
            if hasattr(model, 'set_dropout_scale'):
                model.set_dropout_scale(1.2)  # 0.60 * 1.2 = 0.72 effective
                print(f"[{fold_name}] Dropout boosted to 1.2x scale.")
            # subject_id_map=None disables DANN in train_one_epoch (it checks `is not None`).
            # This is the correct point of control â€” `use_dann` is a local inside the function.
            subject_id_map = None
            print(f"[{fold_name}] DANN disabled (subject_id_map cleared).")

        if epoch == 70 and best_auc < 0.62:
            print(
                f"[{fold_name}] Fold still struggling (best_val_auc={best_auc:.4f} < 0.62 at ep 70). "
                f"Tightening patience to 25."
            )
            early_stopping.patience = 25  # type: ignore[attr-defined]
    ...
    # Filter out any residual SWA keys (n_averaged) that may have leaked into checkpoint
    clean_state = {k: v for k, v in checkpoint["model_state_dict"].items() if k != "n_averaged"}
...
def _run_subject_kfold_training(cfg: TrainConfig) -> None:
    ...
    if n_subjects < 3:
        raise ValueError("L2SO subject-kfold training requires at least three unique subjects.")

    # Deterministic subject ordering for reproducibility
    rng_global = np.random.default_rng(cfg.seed)
    shuffled_subjects = rng_global.permutation(unique_subjects)

    # Build fold groups: one subject per fold for LOSO
    if cfg.num_folds > n_subjects:
        LOGGER.warning("num_folds=%d > n_subjects=%d, capping to %d", cfg.num_folds, n_subjects, n_subjects)
    effective_folds = min(cfg.num_folds, n_subjects)
    fold_subject_groups = [np.array([s], dtype=np.int32) for s in shuffled_subjects[:effective_folds]]
    ...
    for fold_idx, test_subject_ids in enumerate(fold_subject_groups):
        ...
        # =====================================================================
        # P15-FIX: Leave-Two-Subjects-Out (L2SO) Cross-Validation
        # =====================================================================
        # CRITICAL CHANGE: The old approach used stratified holdout from training
        # subjects for validation. This selects checkpoints based on within-subject
        # performance (same subjects, different trials) â€” NOT cross-subject.
        # Result: val_auc=0.85 but test_auc=0.76 (9-point gap).
        #
        # NEW approach:
        #   - Test:  1 full subject (ALL 288 trials) for unbiased evaluation
        #   - Val:   1 full DIFFERENT subject for cross-subject early stopping
        #   - Train: remaining N-2 subjects
        #   - Calib: validation subject (for AdaBN + temperature scaling)
        #
        # This ensures the checkpoint maximizes CROSS-SUBJECT AUC, not
        # within-subject AUC. Expected impact: +10-15% test accuracy.
        # =====================================================================

        test_subject_set = {int(s) for s in test_subject_ids.tolist()}
        non_test_subjects = np.asarray(
            [s for s in shuffled_subjects if int(s) not in test_subject_set],
            dtype=np.int32,
        )

        # Pick the next subject in the same deterministic subject order. When
        # running all 9 BCI folds this makes every subject act as validation
        # exactly once instead of repeating early non-test subjects.
        anchor_positions = [
            pos for pos, subject_id in enumerate(shuffled_subjects)
            if int(subject_id) in test_subject_set
        ]
        anchor_pos = anchor_positions[0] if anchor_positions else fold_idx % n_subjects
        val_subject_id: int | None = None
        for offset in range(1, n_subjects + 1):
            candidate = int(shuffled_subjects[(anchor_pos + offset) % n_subjects])
            if candidate not in test_subject_set:
                val_subject_id = candidate
                break
        if val_subject_id is None:
            raise ValueError("Could not choose a validation subject disjoint from the test subject.")
        val_subject_ids = np.array([val_subject_id], dtype=np.int32)
        train_subject_ids = np.asarray(
            [s for s in non_test_subjects if int(s) != val_subject_id],
            dtype=np.int32,
        )
        ...
        # Calibration = validation subject (used for AdaBN + temperature)
        calib_idx = val_idx.copy()

        print(f"\n[L2SO] Fold {fold_idx + 1}: "
              f"test={test_subject_ids.tolist()}, "
              f"val={val_subject_ids.tolist()}, "
              f"train={train_subject_ids.tolist()} "
        ...
        report = _run_training_session(
            ...
            fold_subject_splits={
                "train": [int(v) for v in train_subject_ids.tolist()],
                "val": [int(v) for v in val_subject_ids.tolist()],
                "calib": [int(v) for v in val_subject_ids.tolist()],
                "test": [int(v) for v in test_subject_ids.tolist()],
            },
        )
